...
def deliver_mail(folder=nil)
  UNIXSocket.open(LDA_SOCK_PATH) do |sock|
...
begin
  ...
  load("/etc/reasonssmf/filter.rb") rescue abort "Failed to read filter file."
  ...
  IO.popen(["sendmail", "-i", "-f", "postmaster", CUR_USER], "w" {|io| io.write MAILCONTENT }
